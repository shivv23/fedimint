...
use fedimint_core::util::{BoxStream, backoff_util};
use fedimint_core::{
    BitcoinHash, OutPoint, TransactionId, apply, async_trait_maybe_send, push_db_pair_items,
    runtime, secp256k1,
};
use fedimint_derive_secret::{ChildId, DerivableSecret};
use fedimint_eventlog::{DBTransactionEventLogExt as _, Event as _};
...
impl WalletClientModule {
    ...
    async fn backfill_receive_operations(&self) -> anyhow::Result<()> {
        ...
        async fn collect_deposit_confirmed_amounts(
            ...
            loop {
                let events = dbtx.get_event_log(cursor, PAGE_SIZE).await;
                if events.is_empty() {
                    break;
                }
        ...
        let deposit_amounts = collect_deposit_confirmed_amounts(&mut dbtx.to_ref_nc()).await?;
